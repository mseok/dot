...
### 핵심 결과
#### Claim 1
[중요한 claim을 plain language로 쓴다.] (Ref: Figure 3b (page 5), Table 2 (page 6); reported number = [absolute number], delta = [vs baseline])
- **Visual**: [[#Figure 3]] (focus: panels b,d)
- **method 연결**: [이 결과가 어떤 설계 선택이나 module 효과를 지지하는가]

[같은 형식으로 claim별로 이어 쓴다. 설명 바로 아래에는 full figure를 반복 embed하지 말고 same-note visual pointer를 둔다.]

### Ablation 및 추가 분석
[무엇을 바꿨는지, 기대 효과가 무엇인지, 실제로 무엇이 일어났는지 쓴다.] (Ref: Figure/Table X (page Y))
- **Visual**: [[#Figure X]] (focus: panels [letter])

### 정성 분석
[그림이 무엇을 보여주는지, 무엇을 봐야 하는지 쓴다.] (Ref: Figure X (page Y))
- **Visual**: [[#Figure X]] (focus: panels [letter])
...
- **본문 연결**: [어느 방법/실험 섹션의 첫 inline 언급과 직접 연결되는가]
...
- **왜 중요한가**: [어떤 결론을 지지하는가]
- **본문 연결**: [어느 claim/분석의 첫 inline 언급과 직접 연결되는가]
...
- [ ] claim 바로 아래에 same-note visual pointer가 있고, panel-level focus가 적혀 있다
