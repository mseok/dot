...
7. 실험은 `figure/table anchor` 중심으로 쓴다.
   ...
   - reference 형식은 최소 `Figure 3 (page 5)`, `Table 2 (page 6)`처럼 번호와 page를 함께 적고, panel-level 근거가 핵심이면 `Figure 3b (page 5)`처럼 panel letter까지 적는다.
   - 설명 바로 아래에는 full figure embed 대신 짧은 visual pointer를 둔다. 예: `Visual: [[#Figure 3]] (focus: panels b,d)`
   - figure/table이 복잡할수록 inline full embed를 반복하지 말고, exact ref와 same-note pointer로 독자를 해당 anchor로 보낸다.
...
8. 결과는 재사용 가능하게 남긴다.
   ...
   - 각 figure/table에는 항상 다음을 적는다.
     ...
     - panel map: a/b/c/... 각 패널이 무엇을 보여 주는지 한 줄씩
     - 왜 중요한가: 어떤 결론을 지지하는가
     - 어떻게 읽는가: 축, 범례, 비교 포인트
...
- claim 문장에 붙는 reference는 가능한 한 exact number + page를 유지하고, panel 근거가 중요하면 panel letter까지 적는다.
- claim 바로 아래에는 가능하면 same-note visual pointer를 두고, inline full embed 반복은 피한다.
